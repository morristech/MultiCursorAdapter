...
public class DBHelper extends SQLiteOpenHelper {
  ...
  private void createTables(final SQLiteDatabase db) {
    ...
    db.execSQL("CREATE TABLE table0 (_id INTEGER PRIMARY KEY AUTOINCREMENT, text TEXT);");
    for (int i = 0; i < 10; i++) {
      db.execSQL(String.format("INSERT INTO table0 VALUES (\"table 0 row %d\");", i));
    }

    db.execSQL("DROP TABLE IF EXISTS table1;");
    db.execSQL("CREATE TABLE table1 (_id INTEGER PRIMARY KEY AUTOINCREMENT, text TEXT);");
    for (int i = 0; i < 10; i++) {
      db.execSQL(String.format("INSERT INTO table1 VALUES (\"table 1 row %d\");", i));
    }
  }
}
